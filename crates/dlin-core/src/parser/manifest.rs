...
pub fn load_manifest_from_bytes(content: &[u8], manifest_path: &Path) -> Result<Manifest> {
    ...
    let unknown_keys = top_level_object
        ...
        .unwrap_or_default();
    enrich_manifest_observations(&mut manifest, Some(unknown_keys), top_level_object);
...
pub fn load_manifest_report_from_bytes(content: &[u8], manifest_path: &Path) -> ManifestLoadReport {
    ...
    enrich_manifest_observations(&mut manifest, Some(unknown_keys), Some(object));
...
fn enrich_manifest_observations(
    manifest: &mut Manifest,
    unknown_top_level_keys: Option<BTreeSet<String>>,
    ...
    let mut capabilities = ManifestCapabilities {
        unknown_top_level_keys: unknown_top_level_keys.unwrap_or_default(),
